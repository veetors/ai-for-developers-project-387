...
## 📊 Прогон 2026-09-02 (регулярный, чистый)

- Дата ISO: `2026-09-02`
- Ветка/commit: `opencode/schedule-df4076-20260902054930` / `e53cb59`
- Стек: тот же `docker compose --profile default` (nginx :3000 → Django → postgres),
  3 URL × 3 прогона, десктопный Lighthouse, assertions — все зелёные, регрессии нет.
- Демо-контент: создан тип события `Консультация 1:1` (id=1) через owner API.
...
Ключевые метрики (медиана): FCP 1.8 с на всех страницах, LCP 2.0–2.4 с, TBT 9–91 мс,
CLS 0, TTI 2.0–2.4 с. Сравнение с прошлым прогоном: `/` и `/event-types` без изменений
(98/98), `/event-types/1` Performance **97 → 96** (колебание в пределах базовой линии,
не регрессия), остальные категории стабильны на 100. Регрессий нет, правок в этот
прогон не требуется.

Публичные отчёты: `/` — <https://storage.googleapis.com/lighthouse-infrastructure.appspot.com/reports/1788328368718-37205.report.html>,
`/event-types` — <https://storage.googleapis.com/lighthouse-infrastructure.appspot.com/reports/1788328368954-77459.report.html>,
`/event-types/1` — <https://storage.googleapis.com/lighthouse-infrastructure.appspot.com/reports/1788328369192-6551.report.html>.

### Топ-10 проблем и как исправить
1. **Единый бандл ~545 КБ** (`unused-javascript`, экономия ~450–600 мс) — код-сплиттинг по маршрутам (`React.lazy`) в `frontend/src/app/providers.tsx`, либо `manualChunks` в `frontend/vite.config.ts`.
2. **Render-blocking CSS `index-*.css`** — ~150 мс на всех страницах. Как исправить: инлайн критического CSS или `preload`/`fetchpriority` ссылки в `frontend/index.html`.
3. **FCP 1.8 с на всех URL** — следствие #1/#2. Как исправить: те же меры (сжатие уже включено; убрать render-blocking и лишний JS).
4. **LCP 2.4 с на `/event-types/1`** — рендер сетки слотов. Как исправить: код-сплиттинг страницы слотов (см. #1).
5. **TBT 91 мс / `max-potential-fid` на `/event-types/1`** — долгая задача при рендере календаря. Как исправить: `React.memo`/лязiness календаря в `frontend/src/components/ui/calendar.tsx` и `frontend/src/features/public-slot-picker/SlotGrid.tsx`.
6. **`interactive` (TTI) 2.4 с на `/event-types/1`** — та же причина, что #4/#5. Как исправить: уменьшить главный бандл (см. #1).
7. **Аудит `network-dependency-tree-insight`** (информационный, диагн. версия) — пересматривать после код-сплиттинга.
8. **Аудит `render-blocking-insight`** (информационный) — пересматривать после #2.
9. **`npm audit` при сборке frontend-образа** — 17 vulns (2 low, 6 moderate, 8 high, 1 critical) в devDeps. Как исправить: `npm audit fix` в `frontend/` (не влияет на Lighthouse, блокирует только при ужесточении политики).
